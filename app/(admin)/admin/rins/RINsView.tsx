...
import type { Cell, RINStatus, RINWithRaised } from "@/types"
...
export function RINsView({ rins, cells }: Props) {
  const [activeFilter, setActiveFilter] = useState<FilterId>("all")

  const visible = activeFilter === "all" ? rins : rins.filter((r) => r.status === activeFilter)
    ...
    <div className="fu">
      ...
      {visible.length === 0 ? (
        <div className="py-16 text-center">
          <p className="font-sans text-[14px] text-[#7A7A7A]">No RINs with this status.</p>
        </div>
      ) : (
