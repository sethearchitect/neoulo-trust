...
import { forwardRIN } from "@/lib/actions"
...
export function LeadRINView({ cellName, rin }: Props) {
    ...
    return (
      <div className="fu">
        ...
        <Card className="p-8 text-center">
          <p className="font-sans text-[14px] text-[#7A7A7A]">No RIN created for your cell yet.</p>
        </Card>
      </div>
    )
  }
  ...
  function handleForward() {
    ...
    startTransition(async () => {
      const result = await forwardRIN(rin!.id)
      if (result.error) setForwardError(result.error)
    })
  }
  ...
  return (
    <div className="fu">
      ...
      {/* Action area */}
      {forwardError && (
        <p className="font-sans text-[13px] text-[#8B2500] mb-3">{forwardError}</p>
      )}
      {rin.status === "funded" && (
        <button
        ...
        >
          {isPending ? "Forwarding…" : "Forward to Neoulo →"}
        </button>
      )}
